...
class Batch {	
	...
	public function listStudents(){
		...
		if(!empty($_POST["search"]["value"])){
			$sqlQuery .= 'WHERE (batch.year LIKE "%'.$_POST["search"]["value"].'%" ';	
			$sqlQuery .= ' OR batch.name LIKE "%'.$_POST["search"]["value"].'%") ';								
		}
		
		if(!empty($_POST["order"])){
			$sqlQuery .= 'ORDER BY '.$_POST['order']['0']['column'].' '.$_POST['order']['0']['dir'].' ';
		} else {
			$sqlQuery .= 'ORDER BY batch.id DESC ';
		}
