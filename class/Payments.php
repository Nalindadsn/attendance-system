...
class Payments {	
	...
	public function listStudents(){
		...
		$sqlQuery = "SELECT payments.id AS id, payments.invoice_no AS invoice_no,  payments.reson AS reson,  payments.created_at AS created_at, payments.amount AS amount,sas_students.id AS s_id, sas_students.name AS sn  FROM payments INNER JOIN sas_students ON payments.student_id = sas_students.id ";
		...
		if(!empty($_POST["order"])){
			...
			$sqlQuery .= 'ORDER BY payments.id DESC ';
		}
		...
		$stmtTotal = $this->conn->prepare("SELECT * FROM payments ");
		$stmtTotal->execute();
